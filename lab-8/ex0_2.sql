...
delimiter | create procedure monthHonorariumPayment(IN monthOfPayment INT, in yearOFpayment INT) procLabel: begin declare countOfCoaches int;
...
INSERT INTO
    tempCoaches(coach_id, monthHourSum)
SELECT
    coach_id,
    SUM(number_of_hours)
FROM
...
IF(ifnull(countOfCoaches, 0) = 0) THEN
SELECT
    'No coaches and hours for this payment period';
...
SELECT
    ...
    tc.monthHourSum,
    c.hour_salary,
    tc.monthHourSum * c.hour_salary,
    monthOfPayment,
    yearOFpayment
FROM
    tempCoaches as tc
    JOIN coaches as c ON tc.coach_id = c.id
WHERE
    tc.id = iterator;
